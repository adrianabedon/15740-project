...
#define NUM_ADDRESS_TABLES_SLOT                                                \
  5000 // this is the number of slots in each address table
       // max size of inner relation in NUM_SLOT * NUM_ADDRESS_TABLES_SLOT
#define NUM_TUPLES 3000
...
typedef struct {
  ...
  hash_t tag;
  // entry point of first tuple in this slot (in the address tables) (log_2_N
  // bits (N is size of first relation))
  atindex_t head;
} slot_t;
...
typedef struct {
  ...
  // 2. Increment its value each time it is used (mod 4 but don't need to worry
  // on FPGA)
